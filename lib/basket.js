 ...
 */
async function basketUuidBelongsToSession( basketUuid, graph ) {
  const basketQuery = `
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX veeakker: <http://veeakker.be/vocabularies/shop/>
    ...
    INSERT DATA {
      GRAPH ${sparqlEscapeUri(graph)} {
        ...
        ${sparqlEscapeUri(basketUri)}
          ...
          veeakker:deliveryType ${sparqlEscapeString("http://veeakker.be/delivery-methods/tour")};
...
async function basketInfo( basketUuid, graph ) {
  const query = await querySudo(`${PREFIXES}
    SELECT ?paymentStatus ?orderStatus ?changedAt ?hasCustomDeliveryPlace ?deliveryPlaceUuid ?deliveryType WHERE {
      GRAPH ${sparqlEscapeUri(graph)} {
        ...
        OPTIONAL { ?s veeakker:deliveryType ?deliveryType. }
...
async function persistDeliveryMeta({ graph, basketUuid, hasCustomDeliveryPlace, deliveryPlaceUuid, deliveryType }) {
    ...
    DELETE {
      GRAPH ${sparqlEscapeUri( graph )} {
        ?basket
          ...
          veeakker:deliveryAt ?deliveryLocation.
      }
    } INSERT {
      GRAPH ${sparqlEscapeUri( graph )} {
        ?basket veeakker:hasCustomDeliveryPlace ${sparqlEscapeBool(hasCustomDeliveryPlace)}.
        ${ deliveryType ? `?basket veeakker:deliveryType ${sparqlEscapeString(deliveryType)}.` : "" }
        ?basket veeakker:deliveryPlace ?newDeliveryPlace.
      }
    } WHERE {
      GRAPH ${sparqlEscapeUri( graph )} {
        ?basket mu:uuid ${sparqlEscapeString(basketUuid)}.
        OPTIONAL { ?basket veeakker:hasCustomDeliveryPlace ?hasCustomDeliveryPlace. }
        OPTIONAL { ?basket veeakker:deliveryType ?deliveryType. }
        OPTIONAL { ?basket veeakker:deliveryAt ?deliveryLocation. }
...
export { ensureBasketExists, basketUuidBelongsToSession, basketInfo,
         basketOrderLines, basketDeliveryAddress, basketInvoiceAddress,
         addOrderLine, removeOrderLine, persistInvoiceAddress, persistDeliveryAddress, persistDeliveryMeta };
