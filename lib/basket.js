...
async function basketOrderLines( basketUri, graph ) {
  const query = await querySudo(`${PREFIXES}
    SELECT ?orderLineUuid ?orderLineAmount ?orderLineOfferingUuid ?orderLineComment WHERE {
      ...
      OPTIONAL {
        GRAPH ${sparqlEscapeUri( graph ) } {
          ?orderLine veeakker:orderedFromShop ?shop.
        }
        GRAPH <http://mu.semte.ch/graphs/public> {
          ?shop mu:uuid ?shopUuid.
        }
      }
    }`);
  ...
  return query.results.bindings.map( (bindings) => {
    ...
    return {
      ...
      shopUuid: bindings.shopUuid ? bindings.shopUuid.value : null
    };
  } );
}
...
async function addOrderLine( { sessionId, basketUuid, shopUuid, basketUri, graph, offeringUuid, amount, comment } ) {
    ...
    INSERT {
      GRAPH ${sparqlEscapeUri(graph)} {
        ...
        ${sparqlEscapeUri(orderLineUri)}
          ...
          ${shopUuid ? `veeakker:orderedFromShop ?shop;` : ""}
          veeakker:hasOffering ?offering.
      }
    } WHERE {
      GRAPH <http://mu.semte.ch/graphs/public> {
        ?offering mu:uuid ${sparqlEscapeString(offeringUuid)}; a gr:Offering.
        ${shopUuid ? `?shop mu:uuid ${sparqlEscapeString(shopUuid)}; a veeakker:Shop.` : ""}
      }
    }`);  
}
  ...
  await updateSudo(`${PREFIXES}
    DELETE {
      ...
      GRAPH ${sparqlEscapeUri(graph)} {
        ...
        OPTIONAL { ?orderLine veeakker:orderedFromShop ?shop. }
      }
    }`);
}
